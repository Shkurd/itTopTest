        ...
        function (result){
            ...
            socket.onmessage = function(event) {
                ...
                if (incomingMessage[0] != undefined || incomingMessage[0] != null) {
                    displayNewNumbersFromServer(incomingMessage);
                }
            ...
            //помещаем новые значения с сервера в соответствующие квадраты и добавляем класс
            function displayNewNumbersFromServer(incomingMessage) {
                    ...
                    for (let i = 0; i < tdArray.length; i++) {
                        for (let j = 0; j < incomingMessage.length; j++) {
                            if (i == incomingMessage[j][0]) {
                                tdArray[i].innerHTML = incomingMessage[j][1];
                                let promise = new Promise((resolve, reject) => {
                                ...
                                promise
                                    .then(
                                        function (result){
                                            ...
                                            tdArray[i].classList.add('active2');
                                        },
                                        function (reject){
                                            tdArray[i].classList.add('active');
                                            tdArray[i].classList.remove('active2');
                                        }
